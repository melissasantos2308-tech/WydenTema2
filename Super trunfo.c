...
int main() {
    ...
    Carta carta_cpu = {"F-22 Lightning", 320, 900, 1800};
...
void desafio_2_multiplos_atributos(Carta c1, Carta c2) {
    ...
    // Exemplo de Estrutura Aninhada (para checar se um jogador tem Potência E Peso altos)
    if (valor1 > valor2) {
        // Vencedor: c1
        if (c1.potencia > 800 && c1.peso < 2000) { // Exemplo com operador lógico '&&'
            printf("BÔNUS: A carta vencedora é de ALTA performance (Potência > 800 E Peso < 2000).\n");
        }
    }
}
...
void desafio_3_operador_ternario(Carta c1, Carta c2) {
    ...
    // Condição de desempate: Peso (Menor vence, se Potência for igual)
    ...
    // Última condição: Potência do Jogador é Menor
    else {
